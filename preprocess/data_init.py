    ...
    if not os.path.exists(join(input_folder, 'sub' + str(sample.Subject).zfill(2), sample.Filename)):
        ...
        if int(sample.ApexFrame) > int(sample.OnsetFrame):
            ...
            if dataset == 'casme2':
                ...
                origin_fname_list = [join('sub' + str(sample.Subject).zfill(2), sample.Filename, 'img' + str(i)) + ftype
                ...
                if startFrame + img_num > int(sample.OffsetFrame):
                    ...
                    for i in range(0, len(origin_fname_list) - 1):
                        ...
                        inter_fname_list[prev_fname] = [prev_fname[:prev_fname.index(ftype)] + "_" + str(i) + ftype for
                                                        i in range(1, inter_frame_num + 1)]
                        if i in range(start_left, end_left):
                            inter_fname_list[prev_fname].append(
                                prev_fname[:prev_fname.index(ftype)] + "_" + str(inter_frame_num + 1) + ftype)
                onset_raw = cv2.imread(join(data_folder, 'data', 'sub' + str(sample.Subject).zfill(2), sample.Filename,
                                            'img' + str(sample.OnsetFrame) + ftype))
                landmarks = np.load(
                    join(data_folder, 'landmarks', 'sub' + str(sample.Subject).zfill(2), sample.Filename + '.npy'))
                os.makedirs(join(input_folder, 'sub' + str(sample.Subject).zfill(2), sample.Filename))
            elif dataset == 'samm':
                startFrame = max(int(sample.ApexFrame) - int(img_num / 2), int(sample.OnsetFrame) + 1)
                endFrame = min(startFrame + img_num - 1, int(sample.OffsetFrame))
                origin_fname_list = [join(str(sample.Subject).zfill(3), sample.Filename,
                                          str(sample.Subject).zfill(3) + "_" + str(i).zfill(5)) + ftype for i
                                     in range(startFrame, endFrame + 1)]
                if startFrame + img_num > int(sample.OffsetFrame):
                    inter_fname_list = {}
                    inter_num = startFrame + img_num - 1 - int(sample.OffsetFrame)
                    inter_frame_num = int(inter_num / (len(origin_fname_list) - 1))
                    ...
                    for i in range(0, len(origin_fname_list) - 1):
                        ...
                        if i in range(start_left, end_left):
                            inter_fname_list[prev_fname].append(
                                prev_fname[:prev_fname.index(ftype)] + "_" + str(inter_frame_num + 1) + ftype)
                onset_raw = cv2.imread(join(data_folder, 'data', str(sample.Subject).zfill(3), sample.Filename,
                                            str(sample.Subject).zfill(3) + "_" + str(sample.OnsetFrame).zfill(5) + ftype))
                landmarks = np.load(
                    join(data_folder, 'landmarks', str(sample.Subject).zfill(3), sample.Filename + '.npy'))
                ...
                endFrame = min(startFrame + img_num - 1, int(sample.OffsetFrame))
                origin_fname_list = [
                    join(str(sample.Subject), sample.Emotion, sample.Filename, 'image' + str(i).zfill(6)) + ftype for i
                    in range(startFrame, endFrame + 1)]
                if startFrame + img_num > int(sample.OffsetFrame):
                    inter_fname_list = {}
                    inter_num = startFrame + img_num - 1 - int(sample.OffsetFrame)
                    ...
                    for i in range(0, len(origin_fname_list) - 1):
                        ...
                        if i in range(start_left, end_left):
                            inter_fname_list[prev_fname].append(
                                prev_fname[:prev_fname.index(ftype)] + "_" + str(inter_frame_num + 1) + ftype)
                onset_raw = cv2.imread(join(data_folder, 'data', str(sample.Subject), sample.Emotion, sample.Filename,
                                            'image' + str(sample.OnsetFrame).zfill(6) + ftype))
            ...
            onset = onset_raw[floor(centre_x - height / 2):floor(centre_x + height / 2),
                    floor(centre_y - height / 2):floor(centre_y + height / 2), :]
            ...
            for i in range(0, len(origin_fname_list)):
                ...
                if os.path.exists(join(data_folder, 'data', fname)):
                    img_raw = cv2.imread(join(data_folder, 'data', fname))
                else:
                    img_raw = cv2.imread(join(data_folder, 'data', origin_fname_list[i - 1]))
                img = img_raw[floor(centre_x - height / 2):floor(centre_x + height / 2),
                      floor(centre_y - height / 2):floor(centre_y + height / 2), :]
                ...
                if inter_num > 0 and i + 1 < len(origin_fname_list) and len(inter_fname_list[fname]) > 0:
                    img_prev = img
                    img_next = cv2.imread(join(data_folder, 'data', origin_fname_list[i + 1]))[
                               floor(centre_x - height / 2):floor(centre_x + height / 2),
                               floor(centre_y - height / 2):floor(centre_y + height / 2), :]
                    inter_img_list = interframes(img_prev, img_next, len(inter_fname_list[fname]))
                    inter_img_list = inter_img_list[
                                     int(len(inter_img_list) / 2) - int(len(inter_fname_list[fname]) / 2):int(
                                         len(inter_img_list) / 2) - int(len(inter_fname_list[fname]) / 2) + len(
                                         inter_fname_list[fname])]
