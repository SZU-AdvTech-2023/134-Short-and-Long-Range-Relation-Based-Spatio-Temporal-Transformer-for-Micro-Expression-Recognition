...
parser.add_argument('-s', '--sub-val', default=6, type=int, metavar='N',
...
def trainer(loso=False, args=None):
    ...
    if args.evaluate:
        ...
        with open('results.csv', 'a') as f:
            print(datetime.datetime.now(), ', sub' + str(args.sub_val).zfill(2) + ', ', str(len(val_loader)) + ', ',
                  res.item() + ', ', top2.item() + ', ', f1.item(), file=f)
        return
